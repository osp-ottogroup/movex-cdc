...
class TableInitializationTest < ActiveSupport::TestCase
  ...
  # One test run for trigger generation and initialization execution
  # A block can be given to by executed after event_log record creation and before prcessing of them
  def run_test
    ...
    yield if block_given?                                                       # Execute tests on Event_Logs
  ...
  test "table_initialization_with_pkey_only" do
    run_with_current_user {
      Table.find(victim1_table.id).update!(yn_payload_pkey_only: 'Y')
      run_test {
        event_logs = Database.select_all "SELECT * FROM Event_Logs WHERE ID > (SELECT MAX(ID) FROM Event_Logs) -2"
        assert event_logs.count > 0, log_on_failure('Should have at least one event log record to test')
        event_logs.each do |e|
          assert !e.payload['CHAR_NAME'], log_on_failure('Payload should not contain the column CHAR_NAME which is not part of the primary key')
        end
      }
