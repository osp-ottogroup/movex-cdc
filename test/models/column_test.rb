...
class ColumnTest < ActiveSupport::TestCase
  ...
  test "select column" do
    ...
    assert columns.count > 0, log_on_failure('Should return at least one column of table')
  end
  ...
  test "count active columns" do
    ...
    assert_equal num_active_4,        Column.count_active(table_id: victim1_table.id),                     log_on_failure('Should return the number of active columns of table')
    assert_equal num_active_4_update, Column.count_active(table_id: victim1_table.id, yn_log_update: 'Y'), log_on_failure('Should return the number of active columns of table for update')
    assert_equal num_active_1_delete, Column.count_active(table_id: tables_table.id, yn_log_delete: 'Y'), log_on_failure('Should return the number of active columns of table for delete')
  end
