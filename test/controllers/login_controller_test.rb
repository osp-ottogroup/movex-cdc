...
class LoginControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should post do_logon" do
    ...
    post "/login/do_logon", params: { email: 'admin', password: MovexCdc::Application.config.db_password }
    assert_response :success

    # login existing user with email downcase
    post "/login/do_logon", params: { email: 'Peter.Ramm@og1o.de'.downcase, password: MovexCdc::Application.config.db_victim_password }
    ...
    post "/login/do_logon", params: { email: 'Peter.Ramm@og1o.de'.upcase, password: MovexCdc::Application.config.db_victim_password }
    case MovexCdc::Application.config.db_type
    ...
    4.downto(0) do                                                              # account is locked after 5 failed logon tries
      ...
      post "/login/do_logon", params: { email: 'Peter.Ramm@og1o.de', password: 'wrong'}
      case MovexCdc::Application.config.db_type
    ...
    user = User.find_by_email_case_insensitive('Peter.Ramm@og1o.de')
    run_with_current_user { user.update(yn_account_locked: 'N') }

    # login  existing user with db-user (admin) in downcase
    post "/login/do_logon", params: { email: MovexCdc::Application.config.db_user.downcase, password: MovexCdc::Application.config.db_password}
    case MovexCdc::Application.config.db_type
    ...
    post "/login/do_logon", params: { email: MovexCdc::Application.config.db_user.upcase, password: MovexCdc::Application.config.db_password}
    case MovexCdc::Application.config.db_type
    ...
    # Non-existing user
    post "/login/do_logon", params: { email: 'Hugo@ottogroup.com', password: 'hugo'}
    assert_response :unauthorized, log_on_failure('Non existing user should not be able to login')

    # redundant db_user
    post "/login/do_logon", params: { email: 'double_db_user', password: 'hugo'}
    assert_response :unauthorized, log_on_failure('Redundant DB user should not used to login')

    # not allowed content
    post "/login/do_logon", params: { email: 'too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_too_large_', password: 'hugo'}
  ...
  test "lock account after 5 attempts" do
    if MovexCdc::Application.config.db_type != 'SQLITE'
      ...
      3.downto(1) do
        post "/login/do_logon", params: { email: MovexCdc::Application.config.db_user.downcase, password: 'wrong password'}
      end
