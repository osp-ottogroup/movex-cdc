...
class TriggerConfigControllerTest < ActionDispatch::IntegrationTest

  # Test Goal: Ensure that the export of all schemas generates the correct json
  test "export" do
    get "/trigger_config", headers: jwt_header(@jwt_admin_token)
    ...
    assert_equal expected_users, actual['users']
    assert_equal expected_schemas, actual['schemas']
  end
  ...
  test "export_schema" do
    ...
    get "/trigger_config", params: {schema: db_user}, headers: jwt_header(@jwt_admin_token)
    assert_response :success
  ...
  test "import_user_update" do
    ...
    assert_no_difference('User.count') do
      post "/trigger_config", headers: jwt_header(@jwt_admin_token), params: {users: [{email: "Sandro.Preuss@ottogroup.com", db_user: Trixx::Application.config.trixx_db_victim_user, first_name: "Grzgorz", last_name: "Pol", yn_admin: "Y"}]}
    end
    assert_response :success
  ...
  test "import_user_create" do
    assert_difference('User.count') do
      post "/trigger_config", headers: jwt_header(@jwt_admin_token), params: {users: [{email: "new@user.hello", db_user: Trixx::Application.config.trixx_db_victim_user, first_name: "Grzgorz", last_name: "Pol", yn_admin: "Y"}]}
    end
    assert_response :success
  ...
  test "import_schema_remove_all" do
    ...
    assert_equal 4, old_schema.tables.count
    assert_equal 1, old_schema.schema_rights.count

    # this is the minimal schema configuration
    schema = [{name: "main", topic: "TheWeather"}, tables: [], schema_rights: []]
    assert_no_difference('Schema.count') do
      post "/trigger_config", headers: jwt_header(@jwt_admin_token), params: {schemas: schema}
    end
    assert_response :success

    new_schema = Schema.find(1)
    assert_equal "main", new_schema.name
    assert_equal "TheWeather", new_schema.topic
    assert_equal 0, new_schema.tables.count
    assert_equal 0, new_schema.schema_rights.count
  end
  ...
  test "import_schema_nothing" do
    ...
    assert_no_difference('Schema.count') do
      post "/trigger_config", headers: jwt_header(@jwt_admin_token), params: {}
    end
    assert_response :success
  ...
  test "import_schema_update" do
    ...
    schema = [{name: "main", topic: "TheWeather",
