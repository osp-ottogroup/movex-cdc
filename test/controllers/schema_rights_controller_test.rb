...
class SchemaRightsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create schema_right" do
    assert_difference('SchemaRight.count') do
      post "/schema_rights", headers: jwt_header(@jwt_admin_token), params: { schema_right: { user_id: sandro_user.id, schema_id: user_schema.id, info: 'Info' } }, as: :json
    end
    assert_response 201

    post "/schema_rights", headers: jwt_header, params: { schema_right: { user_id: sandro_user.id, schema_id: user_schema.id, info: 'Info'  } }, as: :json
    assert_response :unauthorized, log_on_failure('Should not get access without admin role')
  ...
  test "should show schema_right" do
    get "/schema_rights/#{@schema_right.id}", headers: jwt_header(@jwt_admin_token), as: :json
    assert_response :success

    get "/schema_rights/#{@schema_right.id}", headers: jwt_header, as: :json
    assert_response :unauthorized, log_on_failure('Should not get access without admin role')
  end

  test "should update schema_right" do
    patch "/schema_rights/#{@schema_right.id}", headers: jwt_header(@jwt_admin_token), params: { schema_right: { info: 'changed info', lock_version: @schema_right.lock_version } }, as: :json
    assert_response 200

    patch "/schema_rights/#{@schema_right.id}", headers: jwt_header, params: { schema_right: {  } }, as: :json
    assert_response :unauthorized, log_on_failure('Should not get access without admin role')
  end
  ...
  test "should destroy schema_right" do
    ...
    assert_difference('SchemaRight.count', -1) do
      delete "/schema_rights/#{schema_right_to_delete.id}", headers: jwt_header(@jwt_admin_token), params: { schema_right: schema_right_to_delete.attributes}, as: :json
    end
    assert_response 204

    if MovexCdc::Application.config.db_type != 'SQLITE'
      delete "/schema_rights/#{SchemaRight.where(user_id: peter_user.id, schema_id: user_schema.id).first.id}", headers: jwt_header(@jwt_admin_token), params: { schema_right: {lock_version: 42}}, as: :json
      assert_response :internal_server_error
      assert response.body['ActiveRecord::StaleObjectError'], log_on_failure('Should raise ActiveRecord::StaleObjectError')
    end

    delete "/schema_rights/#{@schema_right.id}", headers: jwt_header, as: :json
    assert_response :unauthorized, log_on_failure('Should not get access without admin role')
  end
end
