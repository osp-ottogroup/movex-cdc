...
class ColumnsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create column" do
    assert_difference('Column.count') do
      post "/columns", headers: jwt_header, params: { column: {  table_id: victim1_table.id, name: 'New column', info: 'New info', yn_log_insert: 'Y', yn_log_update: 'Y', yn_log_delete: 'Y'  } }, as: :json
    end
    assert_response 201

    post "/columns", headers: jwt_header(@jwt_no_schema_right_token), params: { column: {  table_id: tables_table.id, name: 'New column', info: 'New info', yn_log_insert: 'Y', yn_log_update: 'Y', yn_log_delete: 'Y'  } }, as: :json
    assert_response :internal_server_error, log_on_failure('Should not get access without schema rights')
  ...
  test "should show column" do
    get "/columns/#{@column.id}", headers: jwt_header, as: :json
    assert_response :success

    get "/columns/#{@column.id}", headers: jwt_header(@jwt_no_schema_right_token), as: :json
    assert_response :internal_server_error, log_on_failure('Should not get access without schema rights')
  end

  test "should update column" do
    patch "/columns/#{@column.id}", headers: jwt_header, params: { column: { yn_lock_delete: 'Y', lock_version: @column.lock_version } }, as: :json
    assert_response 200

    patch "/columns/#{@column.id}", headers: jwt_header(@jwt_no_schema_right_token), params: { column: {  } }, as: :json
    assert_response :internal_server_error, log_on_failure('Should not get access without schema rights')
  end
  ...
  test "should destroy column" do
    ...
    assert_difference('Column.count', -1) do
      delete "/columns/#{column_to_destroy.id}", headers: jwt_header, params: { column: column_to_destroy.attributes}, as: :json
    end
    assert_response 204
    ...
    column_to_destroy = Column.new(table_id: victim1_table.id, name: 'Dummy43', yn_log_insert: 'N', yn_log_update: 'N', yn_log_delete: 'N')
    run_with_current_user { column_to_destroy.save! }
    delete "/columns/#{column_to_destroy.id}", headers: jwt_header, params: { column: {lock_version: 42}}, as: :json
