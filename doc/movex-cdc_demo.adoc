...
Please adjust the environment values according to your setup and set them in each shell you use for the demo. +
Using the file env.sh for these environment settings is suggested because you'll need it multiple times.
----
cat << EOF >env.sh
...
# Because Kafka uses this addresses also from inside the container where 'localhost' has a different meaning
export IP_ADDRESS_KAFKA=192.168.178.34

# URL of existing ORACLE DB
export DB_URL=192.168.178.34:1521/ORCLPDB1
...
=== Run Kafka container
For demo purposes we use an own Docker image for Kafka.
The command will log the Kafka console output and block until termination by Ctrl-C or `docker stop kafka`.
----
. ./env.sh
docker run --rm \
  --name kafka \
  -p 2181:2181 \
  -p 9092:9092 \
  -e KAFKA_HOST_IP=$IP_ADDRESS_KAFKA \
  -e SECURITY_PROTOCOL=PLAINTEXT \
  registry.gitlab.com/osp-silver/oss/movex-cdc/kafka-compact:3.7.0
----

=== Create a Kafka topic
Run the command within the just started Kafka Docker container (`docker exec -ti kafka bash`):
----
$KAFKA_HOME/bin/kafka-topics.sh --create --topic hugo --partitions 4 --bootstrap-server localhost:9092 --replication-factor 1
----

=== Create a consumer process for the topic
Start a new shell and run the command within in the just started Kafka Docker container (`docker exec -ti kafka bash`).
The command will log each new message of the topic and block until termination by Ctrl-C.
----
$KAFKA_HOME/bin/kafka-console-consumer.sh --topic=hugo --bootstrap-server=localhost:9092 --isolation-level=read_committed
----
...
KAFKA_SEED_BROKER: $IP_ADDRESS_KAFKA:9092

################################
EOF
----

=== Pull the Docker mage for MOVEX CDC
----
docker pull docker.io/ottogroupsolutionproviderosp/movex-cdc
----
...
docker run --rm \
  ...
  docker.io/ottogroupsolutionproviderosp/movex-cdc bundle exec rake ci_preparation:create_user
----

=== Run MOVEX CDC Docker container
This will start the container and log the output to the console.
----
docker run --rm \
  --name cdc \
  -e RUN_CONFIG=/etc/run_config.yml \
  -v $PWD/run_config.yml:/etc/run_config.yml \
  -p8080:8080 \
  docker.io/ottogroupsolutionproviderosp/movex-cdc
----

== Prepare test case

=== Create a table to observe for test user
Start a new shell, set the environment values as mentioned above.
Ensure that an Oracle client installation exsists for sqlplus.
----
. ./env.sh
echo "
...
Open the application in browser: `http://localhost:8080` and login with the predefined user "admin" and the password of the MOVEX DB user.
...
* Add name and email,
* Choose an existing DB-user for authentication with its password. +
This can be any DB user including the MOVEX CDC schema owner.
* Check "Admin User" to allow this user administrative tasks
* Click "Create" to establish the user
* Open the user again with the pen icon to add schema rights now
* Add authorized schemas where this user is enabled to configure change tracking
** Select the schema of the defined SOURCE_DB_USER from the list of schemas
...
* Decide whether to include the Oracle transaction ID into the event or not
...
* Choose whether to transfer the current content of the table into the Kafka topic before tracking further changes or not
** Decide if flashback query should be used to initialize with existing records only up to the SCN of trigger creation
...
Now tap on the table to mark it as current, then the configuration of columns appear.
Select the columns you want to transfer to Kafka for the particular operation.

image:config_columns.png[format=png, width=800]

If you want to add filter conditions to the three operations,
then click on the filter icon for the operation and add the filter condition. +
Be aware that these conditions are executed within a trigger, so refer to columns of the table by its qualifier ":new.column" or ":old.column".
...
Now the triggers are activated in the DB  . If requested the initialization tasks are starting in background. +
...
services:
  ksqldb-server:
    ...
    environment:
      ...
      KSQL_BOOTSTRAP_SERVERS: $IP_ADDRESS_KAFKA:9092
