...
If requested in table config, after first generation of trigger a asynchroneous job will be created for transfer of the already existing records of a table to Kafka.
The maximum number of simultaneously processed table initializations is limited by the environment setting TRIXX_MAX_SIMULTANEOUS_TABLE_INITIALIZATIONS.
