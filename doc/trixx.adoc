...
docker run --rm \
  ...
  registry.gitlab.com/otto-group-solution-provider/movex_cdc:master rake ci_preparation:create_user
...
If the amount is not sufficient at runtime than the value of KAFKA_MAX_BULK_COUNT is automatically decreased by the application until it is according to the available memory.|100
...
If this single processing still fails than the event is marked in Table Event_Logs and suspended for processing for the time defined by ERROR_RETRY_START_DELAY.
...
If no further action happens than this event is erased from table 'Event_Log_Final_Errors' by a houskeeping process after FINAL_ERRORS_KEEP_HOURS.
