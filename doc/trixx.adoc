...
This value controls the setting for INI_TRANS for ORACLE.|16
...
|TRIXX_PARTITION_INTERVAL|Interval in minutes between partition changes for table EVENT_LOGS. +
Partition change is used to free already used storage after some minutes and keep the footprint of table EVENT_LOGS as small as possible. +
...
Each worker thread caches the topics of a table or schema for max. 60 seconds before refrehing it with the current configuation.
