...
* The configuration for the tables and columns to observe by triggers is stored local in the database in TriXX' schema.
* This configuration can be maintained manually by TriXX' web-GUI but can also be loaded as JSON file
...
TriXX supports four kinds of message keys for Kafka that can be defined by GUI at table level:
...
TriXX runs on one CPU and 4 GB of memory with it's default settings.
...
TriXX also works for Standard Edition rsp. without partitioning,
...
To ensure sufficient user rights the TriXX schema owner can also be created by TriXX itself with given DB admin credentials.

===== Rights for owner of TriXX DB schema: Oracle =====

.Minimum grants required to operate TriXX with Oracle DB
[cols="~,~"]
|===
|Grant|Description
...
Create TriXX schema owner for Oracle with all required grants by issuing:
[source]
docker run --rm \
  -e TRIXX_KAFKA_SEED_BROKER=/dev/null
  -e TRIXX_DB_TYPE=ORACLE
  -e TRIXX_DB_USER=hugo
  -e TRIXX_DB_PASSWORD=hugo
  -e TRIXX_DB_SYS_PASSWORD=oracle
  -e TRIXX_DB_URL=10.213.131.150:1521/ORCLPDB1
  git.osp-dd.de:5005/main/trixx:master rake ci_preparation:create_user
...
Since the FLASHBACK grant alone does not allow the selection of data from a table without the SELECT grant, this requirement can also be satisfied by granting FLASHBACK ANY TABLE to the TriXX user.
|===
...
|TRIXX_DB_SYS_PASSWORD|Password of DB admin user. Required only for additional maintenance tasks like creation of DB user by TriXX (ci_preparation:create_user)|
|TRIXX_ERROR_MAX_RETRIES|Maximum number of retries after error during transfer to Kafka|5
|TRIXX_ERROR_RETRY_START_DELAY|Number of seconds after error before first retry starts. This delay is tripled for each next retry.|20
|TRIXX_FINAL_ERRORS_KEEP_HOURS|Number of hours final errors are kept in table Event_Log_Final_Errors before erase them by housekeeping|240
...
|TRIXX_KAFKA_COMPRESSION_CODEC|Compression codec used to compress transferred events. Valid values are: 'none' for not using compression or 'snappy' and 'gzip'. The other compression codecs for Kafka 'lz4' and 'tzstd' are not supported by TriXX.|gzip
...
|TRIXX_MAX_SIMULTANEOUS_TABLE_INITIALIZATIONS|Maximum number of simultaneously processed initial transfers of table data after first trigger generation (number of tables)|5
|TRIXX_MAX_SIMULTANEOUS_TRANSACTIONS|Maximum number of transactions simultaneously processing inserts into table EVENT_LOGS without serialization. +
This value controls the setting for INI_TRANS for ORACLE.
Changing this setting requires that there are no pending transactions on table Event_Logs at next startup of TriXX application.
...
|TRIXX_PARTITION_INTERVAL|Interval in seconds between partition changes for table EVENT_LOGS. +
...
Otherwise error ORA-00054 is raised and application does not start.
|60 seconds
|TZ|Sets local timezone within the Docker-container of TriXX-applikation. Must be directly set as environment of container during 'docker run' like '-e TZ="Europe/London"', does not work from config file.|Europe/Berlin
|===
...
===== Docker / Kubernetes =====
The TriXX container produces a continous log output which can become quite large over time.
...
=== Statistics ===
Throughput values ot TriXX are cumulated in the database table "Statistics".
...
Compression is executed once a day as background job in TriXX application.
...
The maximum number of simultaneously processed table initialization jobs is limited by the environment setting TRIXX_MAX_SIMULTANEOUS_TABLE_INITIALIZATIONS.

Precondition for initial transfer of table data is that TriXX-user is allowed to read this table by SELECT, because initial transfer is done directly by selecting from table, not bei trigger execution.
...
exactly one of TriXX' transfer threads is responsible for transfer of all events with the same key. +
...
If this single processing still fails than the event is marked in Table Event_Logs and suspended for processing for the time defined by TRIXX_ERROR_RETRY_START_DELAY.
After a number of not successful retries (defined by TRIXX_ERROR_MAX_RETRIES) the erroneous event is moved to table 'Event_Log_Final_Errors'.

Events moved to final error table can be rescheduled by API function: _TODO: mark API function_

If no further action happens than this event is erased from table 'Event_Log_Final_Errors' by a houskeeping process after TRIXX_FINAL_ERRORS_KEEP_HOURS.
...
|POST|/import_export|JSON object|no|Import configuration data for users and schemas. Each user / schema contained in JSON object creates/replaces the configuration data in TriXX' config tables
|===
...
If want to exactly define the timestamp of change, than: +
