...
The ruby lib remains as fallback alternative for a transition period if problems occur with the Java libs|ruby
...
|SASL_PLAIN|Authentication with username and password. No encryption takes place.
...
===== Using SASL_PLAIN connection to Kafka =====
...
===== Using SASL_SSL connection to Kafka with systems CA certificates =====
...
===== Using SASL_SSL connection to Kafka with own CA certificates in PEM format =====
[source]
KAFKA_SASL_PLAIN_USERNAME: kafka_user
KAFKA_SASL_PLAIN_PASSWORD: kafka_password
...
===== Using SSL/TLS connection to Kafka with CA and client certificates in PEM format =====
...
====== Steps to convert the keystore content generated by Kafka into the PEM formats needed for the ruby-kafka lib of MOVEX CDC ======
...
==== Examples for Kafka configuration ====
TODO: Add examples for Kafka configuration
