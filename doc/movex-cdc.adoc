...
MOVEX CDC supports five kinds of message keys for Kafka that can be defined by GUI at table level:

===== No message key =====
Messages are placed randomly in partitions. MOVEX CDC transmits events in multiple simultaneous threads, therefore sequential order is not guaranteed,
even if the target topic has only one partition.

===== Fixed value =====
A given value is used as key for all produced events. All change events of a table are placed in the same single Kafka partition.

===== Primary key values =====
Ensures that the change history of a single DB record is always consumed in original sequence

===== Transaction-ID =====
Ensures that all events of a particular DB transaction can be consumed in original sequence

===== Expression =====
A SQL expression or SQL statement which returns a single text/char value. This allows to define own strategies for key creation. +
...
If an error occurs during transfer of events to Kafka then MOVEX CDC retries the transfer of the event
