...
|KAFKA_SASL_PLAIN_PASSWORD|Password for authentication with SASL_PLAIN|
|KAFKA_SASL_PLAIN_USERNAME|Username for authentication with SASL_PLAIN|
...
Set only in combination with SSL or SASL_PLAIN.|FALSE
...
|===
==== Connecting Kafka ====
MOVEX CDC supports connections to Kafka authenticated by SASL_PLAINTEXT or SASL_SSL.
...
If the certificates generated for Kafka are in JKS format, the underlying library 'ruby-kafka' of MOVEX CDC does not support this format.
...
For the default PLAIN connection to Kafka without authentication nothing should be done.

For authentication with user and password set the environment parameters KAFKA_SASL_PLAIN_USERNAME and KAFKA_SASL_PLAIN_PASSWORD. +
If KAFKA_SSL_CA_CERT is not explicitely set then KAFKA_SSL_CA_CERTS_FROM_SYSTEM should be set to TRUE to rely on system's CA certificate.
