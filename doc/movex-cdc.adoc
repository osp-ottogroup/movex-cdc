= image:one.o_small.svg[float="left" width=200 ] MOVEX Change Data Capture  =
...
:numbered:
:sectnumlevels: 6
:homepage: https://www.og1o.com
:title-logo-image: one.o.svg
...
MOVEX CDC allows a system-wide identical capture of change events on database tables without redundancy. +
The Kafka-Cluster, which receives events from MOVEX CDC, is responsible for the provision and distribution of events to any consumers (Publish & Subscribe).

=== Concept ===
**********************************************************************
* Database triggers are used to capture change events.
* The configuration for the tables and columns to observe by triggers is stored locally in the database in MOVEX CDC's schema.
...
That means the strategy of key creation influences the horizontal scalability over multiple worker threads and this way the overall transfer bandwidth of your MOVEX CDC instance.
...
MOVEX CDC supports five types of message keys for Kafka that can be defined by GUI at table level:
[cols="~,~"]
|===
...
| Fixed value | A given value is used as key for all produced events. All change events of a table are placed in the same single Kafka partition.
...
|Expression| A SQL expression or SQL statement which returns a single text/char value. This allows to define own strategies for key creation. +
Definition of the expression is done at table level.
...
* A non committed transmission to Kafka can occur several times if transfer is repeated on error. +
Caution: Kafka distinguishes between read_uncommitted and read_committed when consuming.
...
==== Issues that can alter the original order of events  ====
MOVEX CDC guarantees in most cases that events with message key are transferred to Kafka in the same order as they were created in the database. +
But there are some exceptions where the original order of events with the same message key may be altered:

* If a transactions lasts longer until commit and another transaction producing events with the same message key is committed before,
then the possibly younger events are transferred to Kafka before the older events of the still uncommitted transaction. +
This will not be a problem for the message key types "Primary key values" and "Transaction-ID" because the events of a single record or transaction are always transferred in original order.

* If an error occurs during bulk processing of events with the same message key,
then MOVEX CDC retries the transfer of the bulk array by splitting it into smaller chunks of events but keeping the original order (divide & conquer). +
Thus all events are tried to be transferred to Kafka multiple times. +
But if the chunk size is reduced to one and the transfer of a single event still fails, then it is skipped and the following events are tried to transferred to Kafka.

* If an error occurs during transfer of a single event to Kafka, then MOVEX CDC retries the transfer of the event
delayed with ERROR_RETRY_START_DELAY seconds for the first retry. +
If events with the same message key can be processed without any error before the retry of the failed event, then the order of events with the same key may be altered. +
To reduce the likelihood of such situations you can reduce the ERROR_RETRY_START_DELAY to the lowest possible value (e.g. 1 second). +
This ensures that retries are done faster, but may increase the load on the database and Kafka broker thus reducing the throughput.

* If using initialization of table data there is a risk that new changes with a same messsage key are processed to Kafka before the initialization finishes.
...
so processing the change events is completely asynchronous. +
...
Accepting the synchronous overhead of triggers in business transactions the solution is sized for the expected amount of observed change events independent from the total transaction throughput of the entire database.
...
==== Oracle Database ====
Oracle Databases are supported for release 12.2. and higher.
...
- This index ensures processing throughput, but a tiny risk is remaining for wait scenarios at index block split operation under heavy concurrent transactions that are executing MOVEX CDC's triggers.


==== SQLite ====
SQLite is used as a second database for development of MOVEX CDC. It can also be used for productive purposes.
...
==== Sizing of server instance for MOVEX Change Data Capture ====
The application runs on one CPU and 4 GB of memory with its default settings.
But for higher number of worker threads and/or larger memory buffer size you should increase the number or CPUs and memory accordingly. +
By default MOVEX CDC uses up to 75% of the available memory.
If you want to limit the maximum memory used by MOVEX CDC then set JAVA_OPTS=-Xmx to the desired value (like JAVA_OPTS=-Xmx4096m for 4 GB ).


==== Database schema for MOVEX Change Data Capture  ====
The application needs its own database schema at the observed database. +
...
The existence of these grants is checked at application start.
...
Environment variables override values from configuration file.

.Mandatory environment parameters for evaluation at application start
[cols="~,~"]
|===
|Variable|Description

|DB_PASSWORD|Password of DB_USER, is also used as password for user 'admin' at GUI-logon. Therefore also required for database without access control like SQLite.
|DB_TYPE|Defines the type of observed database. Valid values: SQLITE, ORACLE
...
|KAFKA_SEED_BROKER|Comma-separated list of seed-brokers for Kafka logon (Host:Port), Example: "kafka1.og1o.de:9092, kafka2.og1o.de:9092"
|===

.Optional environment parameters for evaluation at application start
[cols="~,~,~"]
|===
...
|INITIAL_WORKER_THREADS|Initial number of worker threads. Each worker threads has its own connection to database and Kafka and operates independent on transferring events from local DB table to Kafka.|3
...
|60
|MAX_WORKER_THREAD_SLEEP_TIME|Max. seconds an idle worker thread may sleep until next lookup for events to process. This value defines the maximum time an event may wait until transfer to Kafka. Smaller values increase the poll rate of transfer workers against the database.|60
...
|SYSTEM_VALIDATION_JOB_CYCLE|Number of seconds between two executions of the system validation job which controls housekeeping and restarts worker threads if needed.|60
...
|TZ|Sets local timezone within the Docker-container of the application. Must be directly set as environment of container like `docker run -e TZ="Europe/London"`. +
...
Therefore a warning is logged each minute if multiple MOVEX CDC instances are detected on the same schema.
...
For authentication at logon, one DB-User is associated with each application user of MOVEX CDC, the password of this DB-user is used for logon.

The application user is authorized for certain schemas for which tables can be tagged for event capturing.
These schemas can be picked from the list of schemas where the user has select grants at at least one table of this schema.
...
* columns of a configured table with marks for Insert/Update/Delete-triggers
...
* choose a value for Kafka key (None / Primary key / Fixed value / Transaction-ID / Expression )
* decide if transaction-ID should be recorded in events (adds approx. 0.3 ms per triggering SQL execution)
* decide if header values according to CloudEvents standard should be set for each event
* decide if only the primary key columns should be sent as event payload or all marked columns (default).
* decide whether the current content of the table should be initially transferred to Kafka at trigger deployment or nor
...
====== Optional additional values from SQL expressions per operation ======
* This feature allows to add additional information to the JSON payload of the event that is not directly available as columns in the changed table.
* These expressions are evaluated inside the trigger and their result is added as an additional column to the event. +
* Each expression SQL must return exactly one formatted string column that is added into the "old" or "new" JSON objects.
...
*Specific SQL expression feature for Oracle*

* There are three flavors of accepted results from additional column expressions:

** JSON objects: used for n:1 relations with multiple single or multiple result columns.
** JSON fragments: are added directly into the JSON message. E.g: `"NAME": "Smith"`
** JSON arrays: used for 1:n relations with multiple result rows.

* JSON objects: the elements of JSON objects are merged into the "old" or "new" JSON object at the first level +
```
...
*Specific SQL expression feature for SQLite*

* The expression result is directly added into the JSON object. +
Therefore the result must contain the complete JSON fragment including name and value like: `"NAME": "Smith"`
or substructures like: `"Address": { "Street": "Main St.", "City": "London" }` or `"Phones": [ "1234", "5678" ]`

```
SELECT '\"ArrayList\": [ '||GROUP_CONCAT('{\"New_Name\":\"'||new.name||'\", \"Old_Name\": \"'||old.name||'\"}', ', ') ||' ]'
```

==== Generation of database triggers per GUI ====
* Select a particular schema or all schemas for the first check for changed trigger configuration
* Generate the code for all triggers where the configuration has changed compared to the existing triggers in DB
* Also load SQLs are generated where initialization of table data is requested
* Mark certain or all tables for deployment of generated triggers and load SQLs
* Hit "Deploy" to immediately create or replace triggers and to execute load SQLs for initial transfer of table data in background
...
===== Kafka =====
List of Kafka error codes is available here: https://kafka.apache.org/protocol#protocol_error_codes
...
|Explicit authorization of transactional id is required, optional as wildcard: +
...
===== Docker / Kubernetes =====
The Docker container of MOVEX CDC produces a continuous log output which can become quite large over time.
You should ensure that logfile size of the Docker container is not unlimited because this may end up in full filesystem. +
For Docker you can configure this behavior in /etc/docker/daemon.json like this:
...
|timestamp|detailed timestamp of triggering event
...
** For initial GUI logon with user "admin" the password is the DB-passwort of MOVEX CDC's DB-user (DB_PASSWORD)
...
Columns of the originating table can be referenced in the initialization filter condition by "<tablename>.<columnname>".
These jobs are queued and processed deferred asynchronously.
...
|Oracle|Flashback query by SCN can be used to select from the table in its state directly after insert trigger check/creation.
...
- There should not be pending transactions for this table at the time of trigger creation because this uncommitted records are not caught later by "SELECT ... AS OF SCN" +
...
Each transfer thread has its own connection to source database as well as to Kafka. +
...
===== Scaling transfer capacity / throughput between MOVEX CDC and Kafka =====
...
if message size is below the magic 1.8K limit for inline storage (no content storage in CLOB).

===== Error handling during transfer =====
Transferring is done with bulk operations against database and Kafka.
If the transfer operation fails the bulk size would be reduced (divide & conquer) until a single event is processed in its own transaction. +
...
If no further action happens then this event is erased from table 'Event_Log_Final_Errors' by a housekeeping process after FINAL_ERRORS_KEEP_HOURS.
...
SELECT FOR UPDATE SKIP LOCKED is used to isolate the concurrent worker threads so an event can be processed by one thread only. +
...
After being completely transferred to Kafka, empty partitions are deleted by the housekeeping job.
So the total size of the table decreases after temporary burst loads in contrast to the high water mark of a common heap table.
...
This adds additional index maintenance load to the triggering transaction and a very small risk of blocking between concurrent transactions at index block split operations.
...
The http-response contains a JSON-object with detailed information.
No authentication is needed to execute the health check.
...
This gives you the choice to maintain the configuration in place via the GUI or managing it externally as a versioned JSON document e.g. in a VCS like git and importing it right before deployment.
...
With schema name in parameter 'schema' the export is limited to a particular schema:
[source]
curl -X GET -H "Authorization: \
`curl -d "email=admin&password=<my_password>" http://localhost:8080/login/do_logon | \
jq .token | sed -e 's/^"//' -e 's/"$//'`" \
http://localhost:8080/import_export/export?schema=HUGO > movex-cdc-config.json
...
This example imports a whole JSON document with one or more schemas:
[source]
...
`curl -d "email=admin&password=<my_password>" http://localhost:8080/login/do_logon | \
jq .token | sed -e 's/^"//' -e 's/"$//'`" \
-d "schema=HUGO&json_data=`cat movex-cdc-config.json | sed 's/"/\\"/g'`" \
http://localhost:8080/import_export/import

CAUTION: If importing a whole JSON document, all schemas and tables that do not occur in this document are purged from the configuration. +
If importing a single schema all tables of that schema that are not contained in the JSON document are purged.
...
table_id_list: optional (default=all), array with IDs from config table 'TABLES', only for this tables triggers will be deployed|JSON object with successful generated triggers and errors +
 +
http response code = 207 (Multi-Status) if DB errors occurred at trigger generation|Check for difference between existing triggers and current configuration, generate and execute the needed DDL statements. Executes for all schemas where the login user has the deployment grant.
...
|POST|/import_export/import|JSON object, schema (Limit exports to a single schema, optional)|no|Import configuration data for users and schemas. Each user / schema contained in the JSON object creates/replaces the configuration data in the applications config tables
...
You can use curl or wget to call API functions with valid authorization by email and password. +
Steps are:

* authenticate with valid user and get JWT token for next steps
* call API method with use of JWT
...
===== API example for trigger deployment without executing DDL (dry run)  =====
```
JWT=`curl -s -d "email=admin&password=<my_password>" http://localhost:8080/login/do_logon | \
jq .token | \
sed -e 's/^"//' -e 's/"$//'`

curl -X POST -H "Authorization: $JWT" -d "dry_run=true" http://localhost:8080/db_triggers/generate_all
```
...
curl -X POST -H "Authorization: $JWT" -d "schema_name=MYSCHEMA" http://localhost:8080/server_control/reprocess_final_errors
```
...
Precondition for DB network encryption is that the DB server is configured in sqlnet.ora with at least "SQLNET.ENCRYPTION_SERVER = ACCEPTED".

==== Encryption of Kafka connection ====
...
To ensure encrypted HTTP traffic you should place MOVEX CDC behind a reverse proxy or ingres controller with SSL encryption. +
...
** The key is given in a file pointed to by environment variable SECRET_KEY_BASE_FILE
...
All database changes made by MOVEX CDC are isolated to this schema (including the generated triggers).
...
.FAQ will be completed over time with all upcoming questions that are not answered before
[cols="~,~"]
|===
|Question|Answer
...
Each worker thread caches the topics of a table or schema for max. 60 seconds before refreshing it with the current configuration.
