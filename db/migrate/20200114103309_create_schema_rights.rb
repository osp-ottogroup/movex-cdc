class CreateSchemaRights < ActiveRecord::Migration[6.0]
  ...
  def change
    create_table :schema_rights do |t|
      t.references  :user,null: false, index: false, comment: 'Reference to user'
      t.references  :schema,null: false, index: { name: "FK_SCHEMA_RIGHTS_SCHEMA"}, comment: 'Reference to schema'
