...
class KafkaBase
  ...
  def topic_attributes_for_describe
    # Available config items according to: https://docs.confluent.io/platform/current/installation/configuration/topic-configs.html
    {
      ...
      'compression.gzip.level'      => { info: "The compression level to use if compression.type is set to gzip. Valid values are between 1 and 9 or -1. Higher values provide better compression at the cost of higher CPU usage. The default value is -1."},
      'compression.lz4.level'       => { info: "The compression level to use if compression.type is set to lz4.. Valid values are between 1 and 17. Higher values provide better compression at the cost of higher CPU usage. The default value is 9."},
      'compression.type'            => { info: "Specify the final compression type for a given topic. This configuration accepts the standard compression codecs ('gzip', 'snappy', 'lz4', 'zstd'). It additionally accepts 'uncompressed' which is equivalent to no compression; and 'producer' which means retain the original compression codec set by the producer."},
      'compression.zstd.level'      => { info: "The compression level to use if compression.type is set to zstd. The default value is 3."},
      ...
      'remote.log.copy.disable'     => { info: "Determines whether tiered data for a topic should become read only, and no more data uploading on a topic. Once this config is set to true, the local retention configuration (i.e. local.retention.ms/bytes) becomes irrelevant, and all data expiration follows the topic-wide retention configuration (i.e. retention.ms/bytes)."},
      'remote.log.delete.on.disable'=> { info: "Determines whether tiered data for a topic should be deleted after tiered storage is disabled on a topic. This configuration should be enabled when trying to set remote.storage.enable from true to false"},
      'remote.storage.enable'       => { info: "To enable tiered storage for a topic, set this configuration as true. You can not disable this config once it is enabled. It will be provided in future versions."},
